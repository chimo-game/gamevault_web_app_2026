...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin }) => {
  ...
  const runProcessingSequence = async () => {
      ...
      const totalSteps = 10;
      const stepDuration = 700; // Increased duration for dramatic effect
      ...
      for (let i = 1; i <= totalSteps; i++) {
          ...
          // Log Logic & Sequence
          if (i === 1) setProcessLog(p => [...p, `> USER DETECTED: ${username.toUpperCase()}`]);
          if (i === 2) setProcessLog(p => [...p, "> ANALYZING NETWORK..."]);
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center w-full">
                                {showPrizeUI ? (
                                    <div className="flex flex-col items-center animate-in zoom-in slide-in-from-bottom-8 duration-500">
                                        <div className="text-kirin-blue font-bold text-[10px] uppercase tracking-[0.2em] mb-3 animate-pulse">Allocating Reward To</div>
                                        ...
                                        <div className="mt-8 space-y-2 w-full max-w-xs">
                                            ...
                                            <div className={`flex justify-between items-center text-xs p-2 rounded border transition-all duration-500 delay-100 ${allocatedPrize > 25000 ? 'bg-purple-500/10 border-purple-500/30 text-purple-400' : 'border-transparent text-gray-600'}`}>
                                    ...
                                    <Terminal className="w-16 h-16 text-kirin-blue animate-pulse opacity-50" />
                                )}
                            </div>
